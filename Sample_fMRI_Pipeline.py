...
#     #fixation1 period
#     bids_dict['onset'].extend(df.fixation1_start.astype(float))
#     bids_dict['duration'].extend(np.abs(df.fixation1_start - df.fixation1_end))
#     bids_dict['trial_type'].extend(['fixation1']*n_trials)
#     bids_dict['video'].extend(df.video.values)
#     ##fixation2 period
#     bids_dict['onset'].extend(df.fixation2_start.astype(float))
#     bids_dict['duration'].extend(np.abs(df.fixation2_start - df.fixation2_end))
#     bids_dict['trial_type'].extend(['fixation2']*n_trials)
#     bids_dict['video'].extend(df.video.values)
    #rating period
...
def GetIndexesOfColumns(regressor_names,keyword='video'):
    return [(idx,regressor_name) for idx,regressor_name in enumerate(column_names) if keyword in str(regressor_name)]

def CreateConfoundsDf(confounds_df):
    #get columns of interest
    motion_outliers = list(filter(lambda x: 'motion_outlier' in x,confounds_df.columns))
    xyz = list(filter(lambda x: ('_y' in x) or ('_x' in x) or ('_z' in x),confounds_df.columns))
    csf_wm = list(filter(lambda x: ('csf' in x) or ('white_matter' in x),confounds_df.columns))
    
    ###fmriprep documentation says not to include cosine regressors if we are doing high-pass filtering  
    #cosine = list(filter(lambda x: 'cosine' in x,confounds_df.columns))
    
    ###Do we need to remove non steady state runs?
    non_steady_state =list(filter(lambda x: 'non_steady_state' in x,confounds_df.columns))
    
    #filter columns based on columns of interest
    #confounds_cols_of_interest = xyz+non_steady_state+motion_outliers
...
def SaveVideoRegressors(column_names,save_dir,fmri_glm,sub_num,ses_num,run_num,fwhm):
    ...
    for idx,vid in idx_vids:
        ...
        save_name ="Participant-{}_session-{}_run-{}_fwhm-{}_beta_{}.nii.gz".format(sub_num,ses_num,run_num,fwhm,vid)
        nib.save(z_map,os.path.join(save_dir,save_name))
        
        
def SaveVideoTimeseries(column_names,save_dir_ts,denoised,sub_num,ses_num,run_num,fwhm):
    #idx_vids = GetIndexesOfColumns(column_names)
    #for idx,vid in idx_vids:
    save_name_ts ="Participant-{}_session-{}_run-{}_fwhm-{}_timeseries.nii.gz".format(sub_num,ses_num,run_num,fwhm)
...
brain_space='space-MNI152NLin6Asym'#'ses-001_desc'#
space_string ='MNI152NLin6Asym'#'space-T1w'#
#fwhm_list=[8]
fwhm_list=[0,2,5,8]
resampled_brain_mask=-1
mask_string =f'*{brain_space}*brain_mask.nii.gz'
anat_path = os.path.join(base_dir_path,
                         f'fmriprep/sub-Participant0{subject_num}/*/anat/')
...
for fwhm in fwhm_list:
    ...
    func_data_path = os.path.join(base_dir_path, f'fmriprep/sub-Participant0{subject_num}/ses-2/func/')
    bold_paths = glob.glob(func_data_path+'*MNI*desc-preproc_bold.nii.gz')

    scan_length = 480 ##8 min scan

    save_dir = os.path.join(base_dir_path,f'FirstLevels/sub-Participant0{subject_num}/Betas_Smoothed_{fwhm}mm/MNI/')
    save_dir_ts = os.path.join(base_dir_path,f'FirstLevels/sub-Participant0{subject_num}/Timeseries_Smoothed_{fwhm}mm/MNI/')
    ...
    for image_path in bold_paths:
        ...
        run_log_file_string =os.path.join(base_dir_path,
                                          'Behavioral',
                                          f'*0{subject_num}*',
        ...
        bids_df.video=[video_str.split('_')[0].split('.')[0] for video_str in bids_df.video]
        video_names = bids_df[bids_df.trial_type=='video'].trial_type +"-"+bids_df[bids_df.trial_type=='video'].video
        ...
        if resampled_brain_mask==-1:
            ...
            resampled_brain_mask = resample_img(brain_mask, affine_func, shape_func[:3],interpolation='nearest')

        #smooth, mask, denoise timeseries
        smoothed = nl.image.smooth_img(scan, fwhm)
        denoised = nl.image.clean_img(smoothed, detrend=False, standardize=False, confounds=confounds_df, high_pass=0.01, t_r=tr, mask_img = resampled_brain_mask)
